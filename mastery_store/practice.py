...
def log_practice_event(student_id: int, question_id: str, kp_ids: list[str], score: float, is_wrong: bool,
                       ques_type: str | None = None, difficulty: str | None = None, d=None, source: str | None = None, ts=None,
                       client_request_id: str | None = None) -> None:
    """插一行答题事件。

    参数：
      student_id, question_id: 学生 / 题目标识
      kp_ids: 该题考察的知识点 id 列表（list[str]，统一标识 kp_id）
      score: 得分率 ∈ [0,1]
      is_wrong: 是否判错（调用方判定，或 score < 0.5 兜底）
      ques_type/difficulty/d: 题型/难度档/难度值（有则填）
      source: exam / homework / classwork
      ts: 答题时间（默认现在）
      client_request_id: 幂等键（可选）；传入则由 (student_id, client_request_id) 唯一索引防重
    """
    if ts is None:
        ts = _utcnow()
...
def get_kp_sequence(student_id: int, kp_id: str, limit: int = 50, wrong_only: bool = False) -> list[dict]:
    """某学生在某知识点（kp_id）上的做题序列（按时间倒序）。

    wrong_only=True 时只返回错题（用于错题本/选错题锚点）。
    """
...
def get_student_events(student_id: int, days=None, wrong_only: bool = False, kp_ids=None, limit: int = 100) -> list[dict]:
    """某学生最近的做题事件。

    可选过滤：days（最近 N 天）、wrong_only（只要错题）、kp_ids（只含考察这些知识点的题，数组重叠）。
    """
    ...
    if kp_ids:
        sql += " AND kp_ids && %s"   # 数组重叠：题的 kp_ids 与给定集合有交集
...
def get_kp_stats(student_id: int, kp_ids: list[str], recent_days: int = 30) -> dict[str, dict]:
    """每个 kp_id 的做题统计（用于学情 per-KP 增强）。

    返回 {kp_id: {practice_count, recent_wrong_count, last_ts}}：
      practice_count       该知识点历史做题总数（不限时间）
      recent_wrong_count   最近 recent_days 天内的错题数
      last_ts              该知识点最近做题时间
    """
    if not kp_ids:
        return {}
...
def get_history_summary(student_id: int, kp_ids: list[str], days: int = 30) -> dict:
...
def seen_question_ids(student_id: int, limit: int | None = None) -> list[str]:
...
def get_latest_results(student_id: int) -> dict[str, bool]:
    """该学生每道题最近一次作答是否错误。

    用 DISTINCT ON 取每道题的最新一条记录，返回 {question_id: is_wrong}。
    用于推荐去重：做对的题排除，做错的题保留（错题重练）。
    学生无记录时返回空 dict。
    """
...
def find_event_by_request_id(student_id: int, client_request_id: str) -> dict | None:
    """按幂等键查是否已处理过。返回 dict(question_id, ts) 或 None。

    submit-answer 在更新掌握度前调用：命中则直接返回，不重复更新/写记录。
    """
